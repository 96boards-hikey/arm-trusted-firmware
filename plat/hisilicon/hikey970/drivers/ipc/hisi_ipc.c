...
static inline void
hisi_ipc_clear_ack(enum src_id source, enum mbox_id mbox)
{
...
static void
hisi_ipc_send_cmd_with_ack(enum src_id source, enum mbox_id mbox,
			   unsigned int cmdtype, unsigned int cmdpara)
{
	...
	mask = (~((int)source | SRC_LPMCU) & IPC_MASK_ALL_SRC);
	/* mask the other cpus */
			...
			enum pm_mode mode)
{
	...
	enum mbox_id mailbox = (enum mbox_id)(LPMCU_MBX0 + core);

	cmdtype = IPC_CMD_TYPE(0, cluster, mode, 0x3);
	cmdpara = IPC_CMD_PARA(0, core);
	source = cluster ? SRC_LCPU : SRC_BCPU;
...
{
	int ret = 0;
	enum mbox_id i = LPMCU_MBX0;

	mmio_write_32(IPC_LOCK_REG, IPC_UNLOCK_VALUE);
	for (i = LPMCU_MBX0; i <= LPMCU_MBX4; i++) {
		mmio_write_32(IPC_MBX_MODE_REG(i), 1);
		mmio_write_32(IPC_MBX_IMASK_REG(i), IPC_MASK_ALL_SRC);
		mmio_write_32(IPC_MBX_ICLR_REG(i), IPC_MASK_ALL_SRC);
	}
